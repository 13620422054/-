...
<script>
    export default {
        ...
        computed: {
            // 定义一个控制全选按钮的属性,这个属性的值与goodsList里的每个商品selected的值相关
            // 如果所有商品的selected为true,那么这个属性值就为true ,否则false
            allSelected(){
        ...
        methods: {
            getGoodList() {
                let ids =Object.keys(this.$store.state.cart); //提取全局状态里的id 在store目录下的car.js里
                this.$http.get(this.$api.shopcartGoods +ids).then(res=>{
                    if(res.data.status==0){
                      
                        res.data.message.forEach(v =>v.selected =true)  //让商品默认就选中
